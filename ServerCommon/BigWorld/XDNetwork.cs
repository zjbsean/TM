...
namespace ServerCommon
{
    namespace Network
    {
        ...
        public class XDNetworkManager
        {
            ...
            public void InitConfig(int outSidePort)
            {
            ...
            {
                m_clientConnMgr.ClientConnClose(m_clientListen, connSession);
                m_doNetworkUpdate = true;
            }
                    ...
                    foreach (var connCheck in m_checkClientConnList)
                    {
                        ...
                        if (connSession != null && connSession.Connected == true)
                        {
                            if (connCheck.isPassedCheck)
                            {
                                ...
                                {
                                    m_clientConnMgr.AddCheckedLongConnection(connSession, connCheck.gameChannleID, connCheck.playerFlagID);
                                    ...
                                    m_clientConnMgr.AddCheckedShortConnection(connSession, connCheck.gameChannleID);
                            }
                            else
                            {
                    ...
                    foreach (var forwardProtocol in m_forwardProtocolList)
                    {
                        ...
                        if (connSession != null)
                        {
                            if (connSession.Connected == true)
                            {
                                ...
                                //SvLogger.Info("forwardProtocol success sessionID=" + forwardProtocol.m_ClientSessionID);
                            }
                            else
                            {
                                SvLogger.Debug("forwardProtocol 链接已关闭 sessionID=" + forwardProtocol.m_ClientSessionID);
                                connSession.Connection.Close();
                        ...
                        {
                            SvLogger.Debug("forwardProtocol 未找到链接 sessionID=" + forwardProtocol.m_ClientSessionID);
                        }
                    }
            ...
            private void dealServerBoardcastProtocolList()
            {
                if (m_serverBoardcastList.Count > 0)
                {
                    ...
                    {
                        if(bp.m_isBoardcastAllPlayer == false)
                        {
                        ...
                        {
                            LinkedList<ClientConnectionsManager.CheckedConnectionInfo> longConnLinkList = m_clientConnMgr.GetAllCheckedLongConnections();
                            LinkedListNode<ClientConnectionsManager.CheckedConnectionInfo> longConnLinkNode = longConnLinkList.First;
                            while(longConnLinkNode != null)
                            {
                                longConnLinkNode.Value.m_ClientSession.Send(bp.m_ProtocolData);

                                longConnLinkNode = longConnLinkNode.Next;
                            }
                        }
                    }
        ...
        public class ClientConnectionsManager
        {
            ...
            public void AddCheckedShortConnection(ConnectionSession connSession, int gameChannelID)
            {
            ...
            public void AddCheckedLongConnection(ConnectionSession connSession, int gameChannelID, long playerFlagID)
            {
                LinkedListNode<CheckedConnectionInfo> checkedNode;
                if (m_checkedClientLongConnLinkNodeDic.TryGetValue(connSession.LinkId, out checkedNode) == false)
                {
                    CheckedConnectionInfo checkedInfo = new CheckedConnectionInfo()
                    {
                        m_ClientSession = connSession,
                        m_GameChannelID = gameChannelID,
                        m_PlayerFlagID = playerFlagID
                    };

                    connSession.EnableHeartBeat(true, true);

                    checkedNode = m_checkedClientLongConnLink.AddLast(checkedInfo);
                ...
                {
                    SvLogger.Error("****** AddCheckedLongConnection Fail : The SessionID Had Added, SessionID={0}", connSession.LinkId);
                }
            }
            ...
            public LinkedList<CheckedConnectionInfo> GetAllCheckedLongConnections() 
            {
                return m_checkedClientLongConnLink;
            }
            ...
            public void ClientConnClose(ListenService clientListen, ConnectionSession connSession)
            {
                switch (connSession.m_ConnType)
                {
                    case ClientProtocolDealManager.eConnectionType.Uncheck:
                        {
                            if (unknowConeectionClose(clientListen, connSession) == false)
                            {
                                if (shortConeectionClose(clientListen, connSession) == false)
                                    longConnectionClose(clientListen, connSession);
                            }
                        }
                        break;
                    ...
                    case ClientProtocolDealManager.eConnectionType.Short:
                        {
                            if (shortConeectionClose(clientListen, connSession) == false)
                            {
                                if (longConnectionClose(clientListen, connSession) == false)
                                    unknowConeectionClose(clientListen, connSession);
                            }
                        }
                        break;
                    case ClientProtocolDealManager.eConnectionType.Long:
            ...
            private bool unknowConeectionClose(ListenService clientListen, ConnectionSession connSession)
            {
            ...
            private bool shortConeectionClose(ListenService clientListen, ConnectionSession connSession)
            {
            ...
            private bool longConnectionClose(ListenService clientListen, ConnectionSession connSession)
            {
                LinkedListNode<CheckedConnectionInfo> checkedConnNode;
        ...
        public class ServerProtocolDealManager
        {
            ...
            {
                public bool m_isBoardcastAllPlayer = false;
